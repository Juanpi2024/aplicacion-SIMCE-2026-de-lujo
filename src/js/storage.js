...
async function pushToCloud(key, data) {
    document.dispatchEvent(new CustomEvent('storage:syncing'));
    try {
        // Map storage keys to Firestore collection names
        const collectionName = key.replace('simce_', '');
        if (Array.isArray(data)) {
            // Only save if items have IDs
            const itemsWithId = data.filter(item => item && item.id);
            if (itemsWithId.length > 0) {
                await FirebaseService.saveItem(collectionName, { id: 'all_data_snapshot', items: itemsWithId });
            }
        } else {
            await FirebaseService.saveItem(collectionName, { id: 'config', ...data });
        }
        document.dispatchEvent(new CustomEvent('storage:synced'));
    } catch (e) {
        console.warn("Cloud sync failed (offline or auth issue):", e);
...
export async function pullFromCloud() {
    ...
    try {
        for (const [key, storageKey] of Object.entries(STORAGE_KEYS)) {
            const collectionName = storageKey.replace('simce_', '');
            const cloudData = await FirebaseService.getAll(collectionName);
            if (cloudData.length > 0) {
                if (collectionName === 'school') {
                    const config = cloudData.find(d => d.id === 'config');
                    if (config) localStorage.setItem(storageKey, JSON.stringify(config));
                } else {
                    const snapshot = cloudData.find(d => d.id === 'all_data_snapshot');
                    if (snapshot && snapshot.items) {
                        localStorage.setItem(storageKey, JSON.stringify(snapshot.items));
                    }
                }
            }
        }
...
export function setSchool(school) {
    ...
    pushToCloud(STORAGE_KEYS.SCHOOL, school);
}
...
export function getPresetBySubject(subject) {
    return isMathematics(subject) ? PRESETS.MATEMATICA : PRESETS.LENGUAJE;
}
