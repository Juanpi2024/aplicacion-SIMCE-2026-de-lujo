...
export function init(navigateTo, showToast) {
  ...
  // Listen for changes in subject to rebuild if we are in Step 3
  const selectAsignatura = document.getElementById('selectAsignatura');
  if (selectAsignatura) {
    selectAsignatura.addEventListener('change', () => {
      if (currentStep === 3) {
        const total = parseInt(document.getElementById('inputTotalPreguntas').value) || 0;
        if (total > 0) buildClaveTable(total, null, selectAsignatura.value);
      }
    });
  }
  ...
  inputFileIA?.addEventListener('change', async (e) => {
    ...
    try {
      ...
      const currentAsignatura = document.getElementById('selectAsignatura').value;
      const isMath = storage.isMathematics(currentAsignatura);
      
      const pautaExtraida = await generarPautaDesdeImagenes(imagenesBase64, totalPreguntas, isMath ? 'Matemática' : 'Lenguaje y Comunicación');
      ...
      if (Array.isArray(pautaExtraida)) {
        ...
        pautaExtraida.forEach(item => {
          ...
          if (contSelect && item.contenido) {
            // Precise matching or fallback
            const options = Array.from(contSelect.options);
            const normalizedContent = item.contenido.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
            
            const exists = options.find(o => {
              const optLabel = o.value.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
              return optLabel === normalizedContent || optLabel.includes(normalizedContent) || normalizedContent.includes(optLabel);
            });
    ...
    if (badgeInfo) {
      badgeInfo.innerHTML = `
        <span style="display:inline-flex; align-items:center; gap:0.4rem; background: rgba(59, 130, 246, 0.08); border:1px solid rgba(59,130,246,0.25); color: var(--info); border-radius:999px; padding: 0.35rem 0.75rem; font-size: var(--fs-xs); font-weight:600;">
          Asignatura detectada: ${preset.asignatura}
        </span>
      `;
    }
