...
import { getSchool, setSchool, getOpenAIApiKey, setOpenAIApiKey, pullFromCloud } from './js/storage.js';
...
function initSyncIndicator() {
  ...
  document.addEventListener('storage:syncing', () => {
    indicator.className = 'cloud-sync-status syncing';
    if (text) text.textContent = 'Sincronizando...';
  });

  document.addEventListener('storage:synced', () => {
    indicator.className = 'cloud-sync-status synced';
    if (text) text.textContent = 'Sincronizado';
    setTimeout(() => {
        indicator.className = 'cloud-sync-status';
        if (text) text.textContent = 'Conectado';
    }, 2000);
  });

  document.addEventListener('storage:error', (e) => {
    indicator.className = 'cloud-sync-status error';
...
async function initApp() {
  ...
  if (synced) {
    updateSchoolBadge();
    renderPage(); // Re-render once cloud data is in
  }
}
